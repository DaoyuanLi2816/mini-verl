...
| 4 | `train.gradient_accumulation_steps` (reduce) | Fewer trajectories held live per optimizer step. | Under `mode: opd` this also changes how on-policy the run is: the trainer emits an `opd_multi_update_warning` when there is more than one optimizer step per rollout batch, because steps after the first are only approximately on-policy. Keep `gradient_accumulation_steps == rollouts_per_cycle` for strict on-policy updates. |
