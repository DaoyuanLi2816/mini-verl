...
- [ ] `bridge doctor` executes zero Python from the inspected bundle by
...
- [ ] A dynamic reward import happens only under
...
- [ ] Adapter safetensors are validated past the header: dtype and shape byte
      ...
      by a header.
- [ ] No input file can be an output file. `import-verl` and `convert-dataset`
      ...
      source.
- [ ] Conflicting miniVERL extension sources fail closed without printing
      extension values; equal duplicates are recorded as deduplication.
- [ ] Invalid rows fail conversion unless `--allow-rejected-rows` is given, and
      a partial run reports `complete_dataset_conversion: false`.
- [ ] Parquet schema checks read footer metadata only, and the dataset scan
      stops reading row groups once its row or byte bound is reached.
- [ ] `python scripts/release_state.py --check` passes: README English and
      ...
      `release-state.yaml`.
- [ ] Every public scientific figure is readable at 390px and the mobile
      readability exemption list is empty.
- [ ] Transactional wording is limited to in-process rollback; no multi-file
      crash atomicity is claimed.
- [ ] Every frozen scientific artifact remains byte-identical.
