...
| current highest-risk work | final-test configuration and analysis are frozen; run all preregistered arms and seeds without inspecting partial outcomes, then publish immutable artifacts and complete the v0.3 release gates |
...
| preregistration correction | revision 1.3 explicitly freezes `offline-kd-oracle.collection_tasks: 64`, documents the defect and requires every arm/seed to restart from fresh cold starts; the replacement final run may start only after this correction is publicly pushed |
| wall-time correction | public revision 1.4 commit `6f4fabbd74cbb5af9c5427fa9d4fcc0d3e9752e7` preserves the complete invalid revision-1.3 wall view (result SHA-256 `425b0fa568f37b09e61af731d3da5009bd3833bdde6efaf2c66e9dba8355cbe`), corrects the eval-calibrated continuation-only target from 50 to 24 seconds, adds a fail-closed publication check, and requires all 3 methods x 3 seeds to rerun after the amendment is public; equal-update and selected-token views remain valid |
