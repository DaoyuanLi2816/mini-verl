...
| focused evidence | regressions cover strict protocol/numerics, run locking, one-shot lifecycle, exact config layers, portable reports and wheel metadata; exact final totals belong only in [`docs/generated/quality.json`](docs/generated/quality.json) |
...
| remaining gate | finish the independent persistence/concurrency/privacy audit, then execute the extracted-sdist, full CI matrix, build, GPU/network and public release gates |
