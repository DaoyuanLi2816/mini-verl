...
def promote(
    ...
    *,
    ...
    if tuple(int(part) for part in qualification.miniverl_version.split(".")[:2]) >= (0, 15):
        from miniverl.qualification_direct import validate_direct_evidence

        if v015_direct is None:
            raise ValueError("v0.15 full qualification requires installed direct-config evidence")
