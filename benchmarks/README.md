...
| --- | --- | --- |
| `results/rollout-runtime-v2-v0.11.0-candidate-rtx4080.json` | WSL2 RTX 4080 16 GB | Exact-wheel qualified candidate: `hf_cached` passed the 2× local gate in every required cell; CUDA Graph vLLM passed the direct-GKD value gate. Both passed refresh, memory and teardown checks. [Result, method and hashes.](../docs/benchmarks/rollout-runtime-v2.md) |
| `results/rollout-runtime-v2-rtx4080.json` | WSL2 RTX 4080 16 GB | Exact-wheel 24-cell `hf_reference` / `hf_cached` / vLLM comparison. vLLM passed the direct-GKD engine gates; `hf_cached` missed the preregistered 2× speed gate, so v0.11.0 remains blocked. [Result, method and hashes.](../docs/benchmarks/rollout-runtime-v2.md) |
| `results/rollout-runtime-v2-hf-reference.json` | WSL2 RTX 4080 16 GB | Pre-v0.11 actor-rollout systems baseline: all 24 preregistered prompt/response/n/sampling cells completed. It freezes throughput and memory for the future `hf_cached` comparison; teacher scoring, updates, full-cycle timing and typed grouped semantics are not measured here. [Method, table and boundaries.](../docs/benchmarks/rollout-runtime-v2.md) |
